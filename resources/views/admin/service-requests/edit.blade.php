    ...
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center">
            <h2>
                ...
                تعديل الطلب: {{ $serviceRequest->name }}
            </h2>
                ...
                <form action="{{ route('admin.service-requests.update', $serviceRequest->id) }}" method="POST">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" 
                                       ...
                                       id="name" 
                                       name="name" 
                                       value="{{ old('name', $serviceRequest->name) }}" 
                                       required>
                                @error('name')
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('service_type') is-invalid @enderror" 
                                    ...
                                    @foreach(App\Models\ServiceRequest::getServiceTypes() as $key => $value)
                                        <option value="{{ $key }}" {{ old('service_type', $serviceRequest->service_type) == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('nationality') is-invalid @enderror" 
                                    ...
                                    @foreach(App\Models\ServiceRequest::getNationalities() as $key => $value)
                                        <option value="{{ $key }}" {{ old('nationality', $serviceRequest->nationality) == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('emirate') is-invalid @enderror" 
                                    ...
                                    @foreach(App\Models\ServiceRequest::getEmirates() as $key => $value)
                                        <option value="{{ $key }}" {{ old('emirate', $serviceRequest->emirate) == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" 
                                           ...
                                           id="is_active" 
                                           name="is_active" 
                                           {{ old('is_active', $serviceRequest->is_active) ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('notes') is-invalid @enderror" 
                                  ...
                                  name="notes" 
                                  rows="4" 
                                  placeholder="أي ملاحظات إضافية حول الطلب...">{{ old('notes', $serviceRequest->notes) }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
